    ...
    <>
      <li className="relative block w-full rounded-lg overflow-hidden">
        <a href={link} target="_blank">
          ...
          {isPast ? <div className="absolute inset-x-0 top-0 ml-2 mt-2 bg-gray-200 w-min px-3 py-0.5 rounded-full text-black font-semibold">
            <p>Past</p>
          </div> : null}
          
          {isToday ? <div className="absolute inset-x-0 top-0 ml-2 mt-2 bg-green-200 w-min px-3 py-0.5 rounded-full text-black font-semibold">
            <p>Today</p>
          </div> : null}

          {!isPast && !isToday ? <div className="absolute inset-x-0 top-0 ml-2 mt-2 bg-purple-200 w-min px-3 py-0.5 rounded-full text-black font-semibold">
            <p>Upcoming</p>
          </div> : null}
